extern "C" __global__ void __launch_bounds__(128) main_kernel0(half* __restrict__ X, half* __restrict__ Y, half* __restrict__ C) {
  ...
  for (int i2_0_0 = 0; i2_0_0 < 128; ++i2_0_0) {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0) {
      *(uint4*)(((half*)buf_dyn_shmem) + (((((ax0_ax1_fused_0 * 1024) + (((int)threadIdx.y) * 256)) + ((((int)threadIdx.x) >> 2) * 32)) + (((((int)threadIdx.x) >> 3) ^ (((int)threadIdx.x) & 3)) * 8)) + 4096)) = X_shared_dyn_local[ax0_ax1_fused_0];
    }
...

  {
    ...
    __asm__ __volatile__(
      ...
      : "=r"(addr)
      : "l"((void *)((&(((half*)buf_dyn_shmem)[(((((((int)threadIdx.y) >> 1) * 2048) + (ax0_0 * 1024)) + (((int)threadIdx.x) * 32)) + 4096)])) + ((((((int)threadIdx.x) & 7) >> 1) ^ i2_0_1) * 8)))
    );
    __asm__ __volatile__(
