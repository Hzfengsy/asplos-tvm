...
class PTXAsyncCopyInjector : public StmtMutator {
 public:
  ...
  Stmt HandleDataCopy(const BufferStoreNode* store, const BufferLoadNode* load,
                      const PrimExpr& pred_guard = Bool(true)) {
    if (load->buffer.scope() == "global") {
      ...
      if (bytes == 4 || bytes == 8 || bytes == 16) {
        ...
        if (indices_lanes == 1) {
          ...
          return Evaluate(
              Call(store->buffer->dtype, tir::builtin::ptx_cp_async_zfill(),
                   {store->buffer->data, tir::Mul(dst_offset, PrimExpr(index_factor)),
                    load->buffer->data, src_offset, PrimExpr(bytes), pred_guard}));
        }
  ...
  Stmt VisitStmt_(const BufferStoreNode* store) {
    if (in_async && (store->buffer.scope() == "shared" || store->buffer.scope() == "shared.dyn")) {
      if (auto* load = store->value.as<BufferLoadNode>()) {
        Stmt new_stmt = HandleDataCopy(store, load);
        if (new_stmt.defined()) {
          return new_stmt;
        }
      } else if (auto* call = store->value.as<CallNode>()) {
        if(call->op==tir::builtin::if_then_else()){
          bool do_rewrite = false;
          PrimExpr else_value = call->args[2];
          
          if (auto* bcast = else_value.as<BroadcastNode>()) {
            if (auto* float_imm = bcast->value.as<FloatImmNode>()) {
              do_rewrite=std::fabs(float_imm->value)<1e-10;
            }
          } else if (auto* float_imm = else_value.as<FloatImmNode>()) {
            do_rewrite = std::fabs(float_imm->value)<1e-10;
          }
          if (do_rewrite) {
            Stmt new_stmt = HandleDataCopy(store, call->args[1].as<BufferLoadNode>(),
                call->args[0]);
            if (new_stmt.defined()) {
              return new_stmt;
            }
