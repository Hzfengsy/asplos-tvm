...
def arm_compute_lib_pattern_table(disabled_ops=["concatenate"]):
    ...
    def check_qnn_conv(extract):
        ...
        if extract.attrs.out_dtype != "uint8" and extract.attrs.out_dtype != "int8":
            return False
        call = extract
    ...
    def check_avg_pool2d(extract):
        ...
        if extract.attrs.dtype != "uint8" and extract.attrs.dtype != "int8":
            return False
        pool = extract.args[0]
...
def qnn_conv2d(expr):
    ...
    if attrs.out_dtype != "int32" and attrs.out_dtype != "":
        return False
    data_typ = args[0].checked_type
    if len(data_typ.shape) != 4 or data_typ.shape[0] != 1 or (data_typ.dtype != "uint8" and data_typ.dtype != "int8"):
        return False
    kernel_typ = args[1].checked_type
    if len(kernel_typ.shape) != 4 or (kernel_typ.dtype != "uint8" and kernel_typ.dtype != "int8"):
        return False
    is_depthwise = is_depthwise_conv2d(
        data_typ.shape,
        attrs["data_layout"],
...
def qnn_dense(expr):
    ...
    attrs, args = expr.attrs, expr.args
    data_typ = args[0].checked_type
    if data_typ.dtype != "uint8" and data_typ.dtype != "int8":
        return False
    kernel_typ = args[1].checked_type
    if len(kernel_typ.shape) != 2 or (kernel_typ.dtype != "uint8" and kernel_typ.dtype != "int8"):
        return False
...
def qnn_add(expr):
    ...
    args = expr.args
    for typ in [args[0].checked_type, args[1].checked_type]:
        if typ.dtype != "uint8" and typ.dtype != "int8":
            return False
