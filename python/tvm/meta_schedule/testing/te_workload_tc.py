...
def conv1d_nlc_f16(  # pylint: disable=invalid-name,missing-docstring
    ...
    weight = te.placeholder((kernel_size, CI // groups, CO), name="weight", dtype="float16")
    ...
    output = te.compute(
        ...
        lambda n, l, co: te.sum(
            (
                ...
                * tir.Cast(value=weight[rl, rc, co], dtype="float32")
            ),
            axis=[rl, rc],
        ),
        name="conv1d_nlc",
...
def conv3d_ndhwc_f16(  # pylint: disable=invalid-name,missing-docstring
    ...
    weight = te.placeholder(
        (kernel_size, kernel_size, kernel_size, CI // groups, CO), name="weight", dtype="float16"
    )
    ...
    output = te.compute(
        ...
        lambda n, d, h, w, co: te.sum(
            (
                ...
                * tir.Cast(value=weight[rd, rh, rw, rc, co], dtype="float32")
            ),
            axis=[rd, rh, rw, rc],
        ),
        name="conv3d_ndhwc",
...
def batch_matmul_nkkm_f16(  # pylint: disable=invalid-name,missing-docstring
    ...
    z = te.compute(  # pylint: disable=invalid-name
        (B, N, M),
        lambda b, i, j: te.sum(tir.Cast("float32", x[b][i][k]) * tir.Cast("float32", y[b][k][j]), axis=[k]),
        name="Z",
    )
    return (x, y, z)
...
def depthwise_conv2d_nhwc_f16(  # pylint: disable=invalid-name,missing-docstring
    ...
    weight = te.placeholder((factor, kernel_size, kernel_size, C), dtype="float16")
    ...
    output = te.compute(
        ...
        lambda n, h, w, c: te.sum(
            (
                tir.Cast("float32", padded[
                    n,
                    h * stride + rh * dilation,
                    w * stride + rw * dilation,
                    c // factor,
                ])
                * tir.Cast("float32", weight[c % factor, rh, rw, c // factor])
            ),
            axis=[rh, rw],
        ),
...
def conv2d_transpose_nhwc(  # pylint: disable=invalid-name,missing-docstring
    ...
    inputs = te.placeholder((N, H, W, CI), name="inputs", dtype="float16")
    weight = te.placeholder((kernel_size, kernel_size, CI, CO), name="weight", dtype="float16")
    ...
    output = te.compute(
        ...
        lambda n, h, w, co: te.sum(
            tir.Cast("float32", _dilate(n, h + rh + border_h, w + rw + border_w, rc))
            * tir.Cast("float32", weight[filter_h - 1 - rh, filter_w - 1 - rw, rc, co]),
            axis=[rh, rw, rc],
        ),
...
def conv2d_capsule_nhwijc_f16(  # pylint: disable=invalid-name,missing-docstring
    N: int,
    H: int,
    W: int,
    CI: int,
    ...
    inputs = te.placeholder((N, H, W, capsule_size, capsule_size, CI), name="inputs", dtype="float16")
    weight = te.placeholder(
        (kernel_size, kernel_size, capsule_size, capsule_size, CI, CO), name="weight", dtype="float16"
    )
    ...
    output = te.compute(
        ...
        lambda n, h, w, cap_i, cap_j, co: te.sum(
            (
                tir.Cast("float32", padded[n, h * stride + rh, w * stride + rw, cap_i, cap_k, rc])
                * tir.Cast("float32", weight[rh, rw, cap_k, cap_j, rc, co])
            ),
            axis=[rh, rw, cap_k, rc],
        ),
...
def norm_bmn_f16(  # pylint: disable=invalid-name,missing-docstring
    B: int,
    M: int,
    N: int,
) -> Tuple[te.Tensor, te.Tensor]:
    a = te.placeholder((B, M, N), name="A", dtype="float16")
    i = te.reduce_axis((0, M), name="i")
    j = te.reduce_axis((0, N), name="j")
    c = te.compute(
        (B,),
        lambda b: te.sum(tir.Cast("float32", a[b][i][j]) * tir.Cast("float32", a[b][i][j]), axis=[i, j]),
        name="C",
    )
...
def conv2d_nhwc_without_layout_rewrite_f16(  # pylint: disable=invalid-name
    Input: int,
    Filter: int,
    stride: int,
    padding: int,
...
def conv2d_nhwc_bn_relu_f16(  # pylint: disable=invalid-name,missing-docstring
    ...
    kernel = te.placeholder((kernel_size, kernel_size, CI, CO), name="kernel", dtype="float16")
    ...
    conv = conv2d_nhwc_without_layout_rewrite_f16(data, kernel, strides, padding, dilation)
    conv = te.compute(
...
def transpose_batch_matmul_f16(  # pylint: disable=invalid-name,missing-docstring
    ...
    query = te.placeholder((batch, seq_len, n_head, n_dim), name="query", dtype="float16")
    value = te.placeholder((batch, seq_len, n_head, n_dim), name="value", dtype="float16")
    query_T = te.compute(
    ...
    out = te.compute(
        ...
        lambda b, h, i, j: te.sum(tir.Cast("float32", query_T[b, h, i, k]) * tir.Cast("float32", value_T[b, h, k, j]), axis=[k]),
        name="C",
    )
    return (query, value, out)


def create_te_workload_f16(name: str, idx: int) -> tir.PrimFunc:
    workload_func, params = CONFIGS_F16[name]
...
CONFIGS_F16 = {
    ...
    "C2D": (
        conv2d_nhwc_f16,
        [
            (32, 16, 16, 64, 64, 3, 1, 1),
            (64, 32, 32, 128, 128, 3, 1, 1)
        ],
    ),
    "C3D": (
        conv3d_ndhwc_f16,
        [
            (32, 16, 16, 16, 64, 64, 3, 1, 1),
            (64, 32, 32, 32, 128, 128, 3, 1, 1)
        ],
    ),
    "GMM": (
        batch_matmul_nkkm_f16,
        [
            (1, 1024, 1024, 1024),
            (1, 2048, 2048, 2048),
            (1, 4096, 4096, 4096),
    ...
    "DIL": (
        ...
        [
            ...
            (32, 16, 16, 64, 64, 3, 1, 1, 2),
            (64, 32, 32, 128, 128, 3, 1, 1, 2),
        ],
    ),
    # "DEP": (
